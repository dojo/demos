define([
	...
	"dojo/request",
	"dojox/rpc/Service"
], function (array, declare, request, Service) {

	var i18nTreeModel = declare(
		"i18nTreeModel",
		null,
	{
		// summary:
		...
		constructor: function(args){
			...
			this.geonames = new Service(require.toUrl("dojox/rpc/SMDLibrary/geonames.smd"));
		},
		...
		getChildren: function(/*dojo.data.Item*/ item, /*function(items)*/ onComplete, /*function*/ onError){
			...
			switch(item.root ?  "top" : item.type){
				case "top":
					var d = request("continents.json", {method: "GET", handleAs: "json-comment-optional"});
				...
				case "continent":
					...
					var callback = function(countries){
						var matches = array.filter(countries, function(c){
							c.type = "country";
				...
				case "country":
					...
					if(!this.d2){
						this.d2 = request("languages.json", {method: "GET", handleAs: "json-comment-optional"});
						this.d2.addCallback(this, function(data){
							...
							return array.filter(data, function(l){
					...
					this.d2.addCallback(function(mapping){
						...
						var ary = array.filter(item.languages.split(","), function(loc){
							return loc;
						});
	
						// map array into locale objects
						var locales = array.map(ary, function(loc){
							loc = loc.toLowerCase();
