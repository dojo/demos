...
if( isset($_FILES[$fieldName])){
	...
	while(isset($_FILES['uploadedfile'.$cnt])){
		...
		if($moved){
			...
			$size = $_FILES['uploadedfile'.$cnt]['size'];
			$file = $upload_path . $name;
			$type = getImageType($file);
			list($width, $height) = getimagesize($file);
			trace("file: " . $file );

			$postdata[$cnt]['file'] = $file;
			$postdata[$cnt]['name'] = $name;
			$postdata[$cnt]['size'] = $size;
			$postdata[$cnt]['width'] = $width;
			$postdata[$cnt]['height'] = $height;
			$postdata[$cnt]['type'] = $type;
		}
		$cnt++;
	}
