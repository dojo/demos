define([
...
], function (domStyle, on, query, request, chartingChart2D, chartingThemesPlotKitBlue, chartingThemesPlotKitCyan, chartingThemesPlotKitGreen) {

	var masterChart = null;
	var initChart = function(){
		masterChart = new chartingChart2D("chart");
	...
	var getResults = function(){
		...
		request("src/results.php", {method: "GET", handleAs:"json"}).then(function(data,ioArgs){
				makeChart(data);
			});
	};
	...
	var makeChart = function(args){
		...
		if(pages==1){ pages++ };
		domStyle.set("chart", "height", pages*180+"px");
	    ...
	    query(".chartHolder").style("visibility","visible");
	
		// if the window resizes, redraw the chart
		if(!windowSizer){
			windowSizer = on(window, "resize", function(){
				c.resize();
			});
		}
	};
