...
dojo.declare('demos.gridx.src.modules.Pagination', _Module, {
	...
	load: function(args, deferStartup){
		//Initialize args
		this._pageSize = args.pageSize || this.grid.pageSize || this._pageSize;
		if(args.initialPage >= 0){
			this._page = args.initialPage;
		}else if(this.grid.initialPage >= 0){
			this._page = this.grid.initialPage;
		}
	...
	filterIndexesInPage: function(indexes, page){
		...
		return first < 0 ? [] : dojo.filter(indexes, function(index){
	...
	_updateBody: function(){
		...
		this.grid.body.setRange(start, size - start < count ? size - start : count);
	},

	_onSizeChange: function(size){
		if(size === 0){
			this._page = 0;
			this.grid.body.setRange(0, 0);
		}else{
